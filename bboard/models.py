...
class Account(models.Model):
    ...
    tel = models.CharField(
        # ! нужно придумать, как для разных отделов/товаров предприятия клиент сможет показывать/скрывать телефон или ссылки. Ведь, он может захотеть для разных сущностей отображать разные контакты
        # ! Перед запуском сайта сделать подобное поле, только для страничек в соцсетях. Отображаться должно в виде анкора (но, нужна какая-то модерация ссылок) (или для сайта)
        # ! для страничек в соцсетях можно добавить поле, которое будет отображаться в виде гиперссылки
        # ! нужно добавить поле для псевдонимов в мессенджерах
        blank=True,
    ...
    prime_places = models.CharField(
        # ! Нужно будет продумать, а, если предприятие должно быть в нескольких прайм-местах одновременно - тогда этого поля будет недостаточно
        blank=True,
    ...
    # Примерно так нужно будет организовать отметки об оплате. Также, в этой функциональности нужно будет учитывать текущий тарифный план и возможно, неполную оплату за месяц. Наверное, это лучше будет сделать в виде отельной таблице на отдельной странице - в таблице будут столбцы тарифа, процента оплаты, успеха оплаты и т. д. Поле успеха оплаты будет выведено на саму страницу магазина.
    #@property
    #def is_overdue(self):
    ...
    # i удаление аккаунта будет осуществляться через запрос. Сначала переводим в статус Неактивный, а затем через какое-то время он будет удаляться
    
    # ? продумаь поле для места деятельности (чтобы можно было указать адрес или номер контейнера)


class Message(models.Model):
    """Повідомлення"""
    # После внедрения системы аутентификации, нужно будет реализовать возможность админу оставлять комментарии одновременно с пользователем
    # Подобное поле нужно сделать для возможности Пожаловаться на товар
...
class Product(models.Model):
    ...
    # поле "Ототображать Телефон/страничка в соцсети/имя в Telegram". Возможно, это поле нужно вынести в отдел магазина, или куда-либо еще. Можно будет сделать несколько таких полей
    
    # акции типа Предложение недели можно организовать как буллево значение здесь в модели товара
    
    # pycoding метод __str__ делает как цепочку почти хлебных крошек через pipe
    
    # можно еще поле доставка буллево добавить
    
    # Преимущественно пусть будет услуга, т. к. продажа товаров может привлечь больше внимания контролирующих органов
    
    # добавить поле Телефон (вдруг на каждый товар разные реализаторы с разными телефонами). И на форме для этого поля сделать предзаполнение как в мозилла
    
    #id = models.BigAutoField(primary_key=True)
    
    # нужно будет добавить поле updated как у Меле. Это и для карты сайта необходимо и для понимания, когда продукт был обновлен
...
class Banner(models.Model):
    """Банер"""
    # Баннер будем выводить самостоятельно из админки, поэтому от аккаунта нужно будет 5 строчек текста и фото с верным соотношением сторон
    # Обязательно добавить атрибут активности для баннера, чтобы менять баннеры из админки, а не из кода
    # (i) выводиться будет только активный баннер (дополнительно в контроллере нужно будет добавть срезы, чтобы не поломать верстку случайным назначением многих баннеров активными). А, вот присутствовать могут многие баннеры в базе. Для того, чтобы не плодить модели, имя баннера должно иметь суффикс например, _top по которому контроллер будет выводить тот или иной баннер в определенной области страницы.
    # (!) серьезный вопрос - куда будут вести анкоры
    # (!) в админке сделать фильтрацию баннеров по магазинам
    name = models.CharField(
